...
\textbf{Definition :}
\[\corrcoef(X, Y) = \frac{\Covhat(X,Y)}{\shat(X)\shat(Y)} \]

\[
\Covhat(X,Y) = \frac{1}{n} {^T}X_c Y_c = \frac{1}{n} <X_c, Y_c>
\]

\[
\shat(X) = \sqrt{\frac{1}{n}{^T}X_cX_c}
\]
\[
= \frac{1}{\sqrt{n}} \sqrt{{^T}X_cX_c} = \frac{1}{\sqrt{n}} ||X_c||
\]

\[
\shat(Y) = \frac{1}{\sqrt{n}} ||Y_c||
\]

\[
\implies \corrcoef(X,Y)= \frac{<X_c, Y_c>}{||X_c||.||Y_c||}
\]
...
\[
\lambda ||v||^2 = \lambda ||v|| + ||v||
\]

\[
\implies - ||u||^2 \leq \lambda ||v|| \leq ||u||
\]

\[
(\lambda ||v||)^2 = \lambda^2 <v, v>
\]
\[
<\lambda v, \lambda v> \; = \; <u_1, u_1>
\]
...
Because $||u||^2 = ||u_1||^2 + ||u_2||^2$ \newline

Consequence : $-1 \leq \corrcoef(X,Y) \leq 1$

\[\Covhat(aX + bY, a'X + b'Y) = \begin{pmatrix}
    a & b
\end{pmatrix} \Covhat(D) \begin{pmatrix}
    a' \\
    b'
